...
import { deriveRoleFlags, resolveStaffMember, shouldBootstrapAdmin } from './auth';
    ...
    const unsubscribe = onAuthStateChanged(auth, async (authUser) => {
      ...
      if (authUser && authUser.email) {
        try {
          ...
          if (!roleData?.role && shouldBootstrapAdmin(authUser.email)) {
          ...
          setStaffMember(
            resolveStaffMember({
              authUser: { email: authUser.email, displayName: authUser.displayName },
              staffDoc,
              roleData: roleData as { role?: AppRole } | null,
            })
          );
        } catch (error) {
