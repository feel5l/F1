...
import { canViewNavItem, getRoleLabel, resolveDisplayRole } from '../lib/authUtils';
...
export default function Layout({ children }: LayoutProps) {
  ...
  const navItems: { name: string; path: string; icon: typeof LayoutDashboard; roles: AppRole[] }[] = [
  ...
  const currentRole = resolveDisplayRole(staffMember?.appRole, isAdmin);

  const visibleNavItems = navItems.filter(item => canViewNavItem(item.roles, currentRole));
